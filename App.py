...
run_model_timestamps={}

investment_slider_marks= {  1:10000,
                            2:20000,
                            3:50000,
                            4:100000,
                            5:500000,
...
# =============================================================================
             dcc.Tab(label='Model', children=[
                #### 1.Row ####                                     
                html.Div([
                    # NNet model selection
                    dcc.Dropdown(
                        id = 'model_selection_dropdown',
                        options=[{'label':'Dense model', 'value':'dense'},
                                 {'label':'LSTM model', 'value':'lstm'}],
                        placeholder = 'Select model...',
                        className = 'six columns'),
                    # Dataset selection
                    dcc.Dropdown(
                        id = 'dataset_selection_dropdown',
                ...
                html.Div([
                    ...
                    html.Div([
                        # Parameters
                        html.Div([
                            dcc.Markdown('Initial investment: '),
                            dcc.Slider(
                                id = 'initial_invest_slider',
                                min = 1,
                                max = 6,
                                marks = {
                                    1:'10k',
                        ...
                        html.Div([
                            html.Button('Run model',
                                id = 'model_run_button'),
                            html.Div(id = 'running_text_parameters'),
                            html.Div(id = 'running_text_model_run'),
                                ],style={'marginTop': 20})
                            ],className = 'three columns',style={'marginTop': 70})
                        ],className = 'row'),
                #### 3.Row ####
                # Portfolio value                                            
                dcc.Graph(
                    id = 'model_portfolio_value_graph',
                    className = 'row')
                    ]),
# =============================================================================
# 3. Tab
...
@app.callback(
            Output('running_text_parameters', 'children'),
             [Input('model_selection_dropdown', 'value'),
             Input('initial_invest_slider', 'value'),
             Input('episodes_slider', 'value'),
             Input('model_train_test_button', 'value')])
def set_parameters(model_selection, invest_value, episode_value, train_test_selection):
    parameters.set_architecture(model_selection)
    parameters.set_initial_invest(investment_slider_marks[invest_value])
    parameters.set_episodes(episode_slider_marks[episode_value])
    parameters.set_mode(train_test_selection)
    
    return 'Parameters: {}'.format(parameters.get_parameters())


@app.callback(
        Output('running_text_model_run', 'children'),
        [Input('model_run_button', 'n_clicks'),
         Input('model_run_button', 'n_clicks_timestamp')])
def run_model(button_clicked, button_time):
    if button_clicked>0:
        run_model_timestamps['{}'.format(button_time)]=button_time
        parameters.run() 
    return 
    

#        parameters = run.RunModel(5, 32, 20000, 'train', None, 'dense')
#        parameters.run()
